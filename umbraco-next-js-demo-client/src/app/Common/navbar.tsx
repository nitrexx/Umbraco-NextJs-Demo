    ...
    <div>
      <nav className="block w-full max-w-screen px-4 py-4 mx-auto bg-white bg-opacity-90 sticky top-3 shadow lg:px-8 backdrop-blur-lg backdrop-saturate-150 z-[9999]">
        <div className="container flex flex-wrap items-center justify-between mx-auto text-slate-800">
          <Link href="/" className="mr-4 block py-1.5 text-red-600 font-bold text-2xl">IP test</Link>

          <div className="lg:hidden">
            <button onClick={toggleMobileMenu} type="button" className="h-6 w-6">
          ...
          <div className={`fixed top-0 left-0 min-h-screen w-64 bg-slate-100 shadow-lg transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"} lg:hidden z-50`}>
            <div className="flex flex-row items-center border-b pb-4">
              <Link href="/" className="text-red-600 font-bold text-xl pt-4 ps-4">NEXTNEWS</Link>
            ...
            <ul className="flex flex-col h-full gap-4 p-4">
              {navItems.map((item, index) => (
                <li key={index} className="text-lg text-slate-600 hover:text-red-500">
                  <Link onClick={() => setIsMobileMenuOpen(false)} href={item.href}>{item.name}</Link>
                </li>
              ))}
              <li className="mt-4">
